...
function get_solar_data() {
    ...
    foreach($db_tables_solar as $table) {
        ...
        if (!$data[$table]['current_data'])
           die("Geen data gevonden voor vandaag! (current_data)");
        ...
        if (!is_numeric($data[$table]['peak_pow']))
           die("Geen data gevonden voor vandaag! (peak_pow)}");

        // Ammount of stuff we collected today
		// SELECT * FROM `stats` WHERE DATE(time) = CURDATE() ORDER BY time ASC LIMIT 1;
        ...
        SELECT `total_pow`
        FROM `$table`
        WHERE DATE(`time`) = CURDATE() ORDER BY time ASC LIMIT 1");
        if (!$today_pow_res) die("Invalid query: " . mysql_error());
        $start_pow = mysql_fetch_object($today_pow_res)->total_pow;
        if (!$start_pow) die("Geen data gevonden voor vandaag! (start_pow)");

        $data[$table]['today_pow'] =
            ($data[$table]['current_data']->total_pow - $start_pow) / 100.;

        // Peak temperature
		$peak_temp_res = mysql_query("
        SELECT `temp`
        FROM `$table`
        WHERE DATE(`time`) = CURDATE()
        ORDER BY `temp` DESC
        LIMIT 1");
        if (!$peak_temp_res) die("Invalid query: " . mysql_error());
        $data[$table]['peak_temp'] = mysql_fetch_object($peak_temp_res)->temp;
        if (!is_numeric($data[$table]['peak_temp']))
           die("Geen data gevonden voor vandaag! (peak_temp)}");
        ...
        // Stuff for year mode
        $year_year = $data[$table]['yearrange']->hyear;
        $data[$table]['year_data'] = get_solar_yearmode($table, $month_year);
...
function get_solar_yearmode($table, $year) {
    $res = mysql_query(
