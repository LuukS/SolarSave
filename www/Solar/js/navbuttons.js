$(function() {
    ...
    $("#year_year_nav_prev").click(function() {
        year_cur_year -= 1;
        update_nav_buttons();
        
        $.getJSON("json.php", {'action': 'year', 'year': year_cur_year}, 
            year_nav_getjson);
        
        return false;
    });
    
    $("#year_year_nav_next").click(function() {
        year_cur_year += 1;
        update_nav_buttons();
...
function year_nav_getjson(data) {
    for (var i = 0; i < data.data.length; i++)
        year_chart.series[i].setData(data.data[i]);
}
...
function update_nav_buttons() {
    ...
    if (year_cur_year == lyear)
        $("#year_year_nav_prev").hide();
    else
        $("#year_year_nav_prev").show();
        
    if (year_cur_year == hyear)
        $("#year_year_nav_next").hide();
    else
        $("#year_year_nav_next").show();
    ...
    $("#year_year_nav_cur").html(year_cur_year);
}
